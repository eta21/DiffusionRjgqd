...
Building on the generalized quadratic framework (see the [__DiffusionRgqd__](https://cran.r-project.org/web/packages/DiffusionRgqd/index.html) package and related documentation), the template SDE for bivariate jump GQDs is given by the eqns:
\[
\begin{aligned}
...
\]
\[
\boldsymbol\sigma(X_t,Y_t,t)\boldsymbol\sigma^{\prime}(X_t,Y_t,t) =
\begin{bmatrix}
...
\dot{z}_{11}&0\\
\dot{z}_{21}&0\\
\end{bmatrix}&\mbox{if type = `Add'}\\&\\
\begin{bmatrix}
\dot{z}_{11}X_t&0\\
\dot{z}_{21}Y_t&0\\
\end{bmatrix}&\mbox{if type = `Mult'}\\
\end{cases}, 
\]
...
\bigg).
\]
In the section that follows, we highlight some of the intricacies of analysisng bivariate jump diffusions using a non-linear diffusion model. 
...
```
For purposes of the illustration we simulate trajectories for the process and superimpose the simulated paths on a countour plot of the transitional density along various points along the transition horizon. In order to illustrate the dichotomous  nature of the process, we will label each simulated trajectory according to wheteher a jump has or has not occured along its trajectory. 
