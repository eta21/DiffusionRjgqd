...
__DiffusionRjgqd__ is built around an algorithm that approximates solutions to the Kolmogorov forward eqn
\[
\begin{aligned}
\frac{\partial}{\partial t} f(X_t|X_s) =& -\frac{\partial}{\partial X_t} [\mu(X_t,t) f(X_t|X_s)]+\frac{1}{2}\frac{\partial^2}{\partial X_t^2} [\sigma^2(X_t,t) f(X_t|X_s)]\\ &+\int\lambda(X_t-J(X_t,\dot{z}_t))f(X_t-J(X_t,\dot{z}_t)|X_s)d\phi(\dot{z}_t)d\dot{z}_t\\
&-\int\lambda(X_t)f(X_t|X_s)d\phi(\dot{z}_t)d\dot{z}_t, 
\end{aligned}\]
where $\phi(.)$ gives the jump distribution, which governs the transitional densities of jump diffusion processes. This is achieved by calculating the conditional moment trajectories of a diffusion process over time and subsequently carrying these moments into a suitable surrogate density. 
...
&+\sum_{l=1}^2\lambda_l(t)\bigg(\sum_{k=0}^i {i\choose k}m_{k+l-1}(t) u_{i-k+l}^{(0)}(t)-m_{i+l-1}(t) \bigg),\\
\end{aligned}
\]
subject to the initial conditions $m_i(t) = X_t^i$  for $i = 1,2,\ldots$, where $u_{i}^{(0)}$ denotes the $i$-th non-central moment of the random variable $\dot{z}_t$. By solving these equations numerically via standard Runge-Kutta schemes and plugging the resulting values into a  surrogate density such as a Normal distribution or a saddlepoint approximation, it is possible to accurately approximate the transitional density.
...
where $f_J(X_t|X_s)$ denotes the transitional density of the jump diffusion, $f_D(X_t|X_s)$ denotes the purely diffuse counterpart to the jump diffusion and $f_E(X_t|X_s)$ denotes an excess distribution, and $P(N_t-N_s=0)$ is calculated as the solution to the equation:
...
An example pf non-linear jump diffusion model often used in finance, termed the [``basic affine jump diffusion'' (BAJD)](https://en.wikipedia.org/wiki/Basic_affine_jump_diffusion) (Eckner 2009) extends upon the popular [CIR process](https://en.wikipedia.org/wiki/Cox-Ingersoll-Ross_model) by assuming that the paths of the process undergoes exponentially distributed jumps that occur with constant intesity. Thus, the corresponding SDE is given by the system:
...
```
Note that, since the transition horizon is large we have not specified that the excess factorization need be used. If we wish to envoke the excess factorization we can specify this through the `factorize` parameter:
```{r fig.align = 'center'}
 Xt <- seq(5.5,8,1/50)
 t  <- 0.2
...
```
For comparitive purposes we may compare simulated trajectories of the moments of the process and compare the simulated trajectory of the transitional density to that of the approximation. This can be achieved using a (somewhat oversimplified) simulation algorithm:
...
Now, we can do a similar experiment over a short transition horizon. For purposes of the comparison we will shhrink the diffusion parameter a bit in order to exaggerate the effect of the jump mecanism:
...
Eckner, A. (2009). ``Computational techniques for basic affine models of portfolio credit risk''.
*Journal of Computational Finance*, __13__(1):63.

--------

# Further reading
* [Methodology paper](DiffusionRjgqd_Paper.pdf) and [replication materials](DiffusionRjgqd_RepMaterials.html).
* [Package manual](https://cran.r-project.org/web/packages/DiffusionRjgqd/DiffusionRjgqd.pdf)
* Related packages:
    + [__DiffusionRgqd__](https://cran.r-project.org/web/packages/DiffusionRgqd/index.html)
* More Vignettes:
