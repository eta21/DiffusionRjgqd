JGQD.estimates = function(x,thin = 100, burns)
{
  if(class(x)=='JGQD.mle')
  {
    ...
    EstCI <- data.frame(Estimate=x$opt$par, xower_95=lower,Upper_95=upper)
   ...
   if(class(x)=='JGQD.mcmc')
  {
     ...
     dat=data.frame(cbind(round(cbind(est,CI),2),'|',round(cor(x$par.matrix[windw,]),2)))
